...
class Pathfinder:
    ...
    def get_distance(self, x1: int, x2: int, y1: int, y2: int) -> int:
    ...
    def find_path(self,start_x: int, start_y: int, target_x: int, target_y: int) -> Optional[List[Tuple[int, int]]]:
    ...
    def reconstruct(self: dict, came_from, current: tuple[int,int]) -> list[tuple[int,int]]:
        path = []
